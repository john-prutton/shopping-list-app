...
import { items, type Item, type NewItem, ItemId } from "@/lib/db/schema/items"
...
export const deleteItem = async (itemId: ItemId) => {
	// Check if user is authed
	const { session } = await getUserAuth()
	...
	// Finally delete item
	try {
		const {} = await db.delete(items).where(eq(items.id, itemId))
